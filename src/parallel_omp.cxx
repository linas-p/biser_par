...
int main (int argc, char ** argv) {
    ...
    int size_N = 8, N = size_N, i, n = 1;
    /* Last argument matrix size N */
    if (argc > 1) {
        std::cout << "Parsed parameter N = " << std::stoi(argv[argc-2]) << std::endl;
        std::cout << "Threads number n = " << std::stoi(argv[argc-1]) << std::endl;
        size_N = std::stoi(argv[argc-2]);
        N = size_N;
        n = std::stoi(argv[argc-1]);
    }
    int numberOfThreads = n, idThread;
    ...
    #pragma omp parallel private(idThread)
    {
        ...
        //std::cout << "?? " << N << " ?? \n" << idThread << " n: " << numberOfThreads << std::endl;
        calc_ref(N, idThread);
    }
    #pragma omp barrier
    ...
    double time = ((double)(end - begin)) * 1000.0;
    std::cout << "sugaista: " << time << std::endl;
    std::ostringstream file_name;
    file_name << "../output/omp_N_"<< size_N << "_p_" << numberOfThreads;
